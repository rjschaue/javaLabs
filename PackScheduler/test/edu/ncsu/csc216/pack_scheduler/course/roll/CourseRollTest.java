...
public class CourseRollTest {
	...
	public void testEnroll() {
		...
		try {
			roll = new CourseRoll(251);
		} catch (IllegalArgumentException e) {
			assertTrue(roll == null);
		}
		
		//Construct a new CourseRoll
		roll = new CourseRoll(10);
		...
		//Try enrolling a dupliate student
		try {
			roll.enroll(s1);
			fail();
		} catch (IllegalArgumentException e) {
		...
		//Try enrolling a student past capacity
		try {
			roll.enroll(s11);
			fail();
		} catch(IllegalArgumentException e) {
	...
	public void testDrop() {
		...
		//Drop valid students in the course roll
		roll.drop(s3);
		assertTrue(roll.getOpenSeats() == 8);
